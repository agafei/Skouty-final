...
if (is_post_request()) {
  ...
  if (empty($errors)) {
    $user = mysqli_fetch_assoc(find_driver_by_email($email));
    
    if ($user && password_verify($password, $user["password"])) {
      
      $user["user_type"] = DRIVER_TYPE;
      $user["user_id"] = $user["driver_id"];
      log_in_user($user);
      redirect("/driver");
      exit;
...
if (is_logged_in() && $_SESSION["user_type"] == DRIVER_TYPE) {
  redirect("/driver");
}
$page_title = "Log in";
$jumbotron_title = "Driver Login";
$jumbotron_subtitle = "";
        ...
        <div class="col-lg-12">
            <div class="main bg-white mb-4 p-4">
                ...
                <form method="post">
                    ...
                    <a class="mx-4" href="/driver/signup">New here? Create an account</a>
                </form>
                </div>
            </div>
        </div>
